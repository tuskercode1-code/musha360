    ...
    <main className="bg-white min-h-screen">
      ...
      <section className="relative min-h-[90vh] flex items-center justify-center bg-slate-950 overflow-hidden py-24 md:py-20">
        ...
        <div className="relative z-10 text-center px-6 w-full max-w-6xl mt-12">
          ...
          {/* SEARCH BAR */}
          <form action="/" method="GET" className="bg-white p-2 rounded-[2.5rem] md:rounded-full shadow-2xl flex flex-col lg:flex-row items-center gap-1 border border-white/10 backdrop-blur-md">
            ...
            <div className="relative flex items-center px-6 gap-3 w-full lg:w-auto min-w-[180px] border-b lg:border-b-0 lg:border-r border-slate-100">
              ...
              <select name="type" defaultValue={typeFilter} className="w-full py-5 bg-transparent outline-none font-bold text-slate-800 appearance-none cursor-pointer pr-8 uppercase tracking-widest text-[10px]">
            ...
            <div className="relative flex items-center px-6 gap-3 w-full lg:w-auto min-w-[140px] border-b lg:border-b-0 lg:border-r border-slate-100">
              <select name="listing" defaultValue={listingFilter} className="w-full py-5 bg-transparent outline-none font-bold text-blue-600 appearance-none cursor-pointer text-center uppercase tracking-widest text-[10px]">
            ...
            <div className="relative flex items-center px-6 gap-3 w-full lg:w-auto min-w-[180px]">
              ...
              <select name="max_price" defaultValue={maxPriceFilter} className="w-full py-5 bg-transparent outline-none font-bold text-slate-800 appearance-none cursor-pointer pr-8 uppercase tracking-widest text-[10px]">
      ...
      {/* LISTINGS GRID */}
      <section id="listings" className="max-w-7xl mx-auto px-6 py-20 md:py-24 scroll-mt-20">
        <h2 className="text-4xl md:text-5xl font-black text-slate-900 tracking-tighter uppercase italic mb-12 md:mb-16">
            ...
            return (
              <div key={property.id} className="group block relative">
                <Link href={`/property/${property.id}`}>
                  <div className="relative aspect-[4/3] rounded-[2.5rem] md:rounded-[3rem] overflow-hidden mb-6 md:mb-8 shadow-sm group-hover:shadow-2xl transition-all duration-500 border border-slate-100 bg-slate-50">
                    ...
                    <div className="absolute top-6 md:top-8 left-6 md:left-8 bg-slate-900/90 backdrop-blur-md px-4 md:px-5 py-2 rounded-full text-[9px] md:text-[10px] font-black uppercase tracking-[0.2em] text-white">
                ...
                <Link href={`/property/${property.id}`}>
                  <div className="px-2 md:px-4">
                    <h3 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tighter mb-1">
                      ...
                      {property.listing_type === 'RENT' && <span className="text-sm md:text-base text-slate-400 font-bold ml-1 uppercase">/mo</span>}
                    </h3>
                    <p className="text-lg md:text-xl font-bold text-slate-800 mb-3 tracking-tight leading-none uppercase">{property.title}</p>
                    <p className="text-slate-400 font-bold flex items-center gap-2 text-[11px] md:text-sm uppercase tracking-wide">
